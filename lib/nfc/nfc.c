...
static int32_t nfc_worker_listener(void* context) {
    ...
    while(true) {
        // The listener worker's main job IS to wait, so it doesn't hold the lock here.
        // It acquires the lock inside the specific transaction functions like nfc_listener_tx.
...
static int32_t nfc_worker_poller(void* context) {
    ...
    bool exit = false;
    while(!exit) {
        // The handlers themselves should not hold locks while waiting for user input.
        // Locks will be placed inside the specific transaction functions.
...
void nfc_config(Nfc* instance, NfcMode mode, NfcTech tech) {
    ...
    FURI_LOG_I(TAG, "Configuring mode: %s, tech: %d", (mode == NfcModePoller) ? "Poller" : "Listener", tech);
...
NfcError nfc_iso14443a_poller_trx_custom_parity(
    ...
    do {
        ...
        if(error != FuriHalNfcErrorNone) { ret = nfc_process_hal_error(error); break; }
        ...
        error = furi_hal_nfc_poller_rx(
            instance->rx_buffer, sizeof(instance->rx_buffer), &instance->rx_bits);
        if(error != FuriHalNfcErrorNone) { ret = nfc_process_hal_error(error); break; }
        if(instance->rx_bits >= 9 && (instance->rx_bits % 9) != 0) { ret = NfcErrorDataFormat; break; }
...
NfcError nfc_iso14443a_listener_tx_custom_parity(Nfc* instance, const BitBuffer* tx_buffer) {
    ...
    FuriHalNfcError error = furi_hal_nfc_iso14443a_listener_tx_custom_parity(tx_data, tx_parity, tx_bits);
    ret = nfc_process_hal_error(error);
    furi_hal_nfc_release();
    return ret;
}
