...
void subghz_txrx_hopper_update(SubGhzTxRx* instance, float stay_threshold) {
    ...
    if(instance->txrx_state == SubGhzTxRxStateRx) {
        // Reset decoder state and worker filter accumulator
        ...
        // Fast frequency hop: all CC1101 operations in a single SPI session
        // with TIM2 interrupts masked.  Avoids the full stop/start async_rx
        // cycle that tears down and rebuilds TIM2 (6 separate SPI sessions
        // + thread join/create ≈ 10-20 ms blocking the GUI thread).
        furi_hal_subghz_async_rx_hop(instance->preset->frequency);
    }
}
